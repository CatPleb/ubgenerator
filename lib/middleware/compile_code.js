...
var wstream = exports.wstream = function (fileloc, packages, latexcode, callback) {
  ...
  try{
    ...
    wstream.write('\\usepackage[utf8]{inputenc}'+               //defining packages used everytime
                  '\\usepackage{amsmath}\n'+
                  '\\usepackage{amsthm}\n'+
                  '\\usepackage{amssymb}\n'+
  ...
  outstream.on('data', (chunk) => {
    ...
    if ( startindex > -1) {
      ...
      if ( endindex > -1) {
        ...
        latex_error = '<br>Something went wrong. There probably is a wrong backslash command somewhere in your code.';
      }
    }
  });
...
exports.final_compile = function (packages, latexcode, callback) {
  ...
  tmp.dir({dir: './temporary', unsafeCleanup: true}, function _tempDirCreated(err, tmppath, cleanupCallback) {
    if (err) throw err;
